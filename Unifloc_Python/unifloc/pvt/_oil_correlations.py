...
class OilCorrelations:
    ...
    def __init__(self, oil_correlations):
        ...
        if oil_correlations["pb"] == "standing":
            ...
            raise exc.NotImplementedPvtCorrError(
    ...
    def __oil_fvf_vasquez_above(p, compr, pb, bob):
        ...
        -------
        """
        oil_fvf_vasquez_above = bob * np.exp(compr * const.PSI * (10**6) * (pb - p))
    ...
    def __oil_compr_vasquez(t, gamma_oil, gamma_gas, rsb, p):
        ...
        return max(
            (28.07291666666667 * rsb + 30.96 * t - 1180 * gamma_gas + 1784.3149999999998 / gamma_oil - 10992.855)
            / (100000 * p * const.PSI * (10**6)),
            2 * 10**-12,
        )
    ...
    def __oil_deadviscosity_beggs(gamma_oil, t):
        ...
        if t < 70:
            ...
            oil_deadviscosity_beggs = 10 ** (np.log10(b) - c * np.log10(t))
        else:
    ...
    def _oil_viscosity_vasquez_beggs(oil_liveviscosity, p, pb):
        ...
        -------
        """
        m = 957 * (p * 10 ** (-6)) ** 1.187 * np.exp(-11.513 - 1.302e-08 * p)
    ...
    def __calc_st_oil_gas_baker(t, gamma_oil, rs):
        ...
        rs *= 5.614583333333333
        sto = (
            (1.17013 - 0.001694 * t)
            * (38.085 - 0.259 * (141.5 / gamma_oil - 131.5))
            * (0.056379 + 0.94362 * np.exp(-0.0038491 * rs))
        )
        return sto / 1000
